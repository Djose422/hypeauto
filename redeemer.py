...
class HypeRedeemer:
    ...
    async def verify_pin(self, pin: str) -> dict:
        ...
        Solo ingresa el PIN y clickea Validar. No llena game ID ni canjea.
        Returns: {'pin_used': bool, 'pin_valid': bool, 'detail': str, 'error': bool}
        """
        context = None
        page = None
        try:
            async with self._semaphore:
                context, page = await self._get_page()
                logger.info(f"[verify][{pin[:8]}...] Verificando PIN")
                ...
                # Click Validar e interceptar respuesta
                try:
                    async with page.expect_response(
                        lambda r: "/validate" in r.url and "account" not in r.url,
                        timeout=15000
                    ...
                    validate_resp = await resp_info.value
                    if validate_resp.status >= 400:
                        logger.info(f"[verify][{pin[:8]}...] HTTP {validate_resp.status} → PIN inválido/expirado")
                        return {'pin_used': True, 'pin_valid': False, 'detail': f'HTTP {validate_resp.status}', 'error': False}
                except Exception:
                    pass

                # Esperar card flip (producto aparece = PIN válido) o error
                try:
                    await page.wait_for_selector(".card.back .body", state="visible", timeout=10000)
                    # Card flip apareció → PIN es válido, no fue canjeado
                    product_name = ""
                    try:
                        product_el = await page.query_selector(".product-header h2")
                        if product_el:
                        ...
                        pass
                    logger.info(f"[verify][{pin[:8]}...] PIN VÁLIDO (no canjeado) → {product_name}")
                    return {'pin_used': False, 'pin_valid': True, 'detail': product_name, 'error': False}
                except Exception:
                    # No apareció card flip → buscar mensaje de error
                    error_el = await page.query_selector(".text-danger, .error-message, .alert-danger")
                    if error_el:
                        error_text = (await error_el.text_content()).strip()
                        if any(w in error_text.lower() for w in ["canjeado", "redeemed", "usado", "used"]):
                            logger.info(f"[verify][{pin[:8]}...] PIN YA CANJEADO: {error_text}")
                            return {'pin_used': True, 'pin_valid': False, 'detail': error_text, 'error': False}
                        if any(w in error_text.lower() for w in ["expirado", "expired", "vencido"]):
                            logger.info(f"[verify][{pin[:8]}...] PIN EXPIRADO: {error_text}")
                            return {'pin_used': True, 'pin_valid': False, 'detail': error_text, 'error': False}
                        logger.warning(f"[verify][{pin[:8]}...] Error desconocido: {error_text}")
                        return {'pin_used': True, 'pin_valid': False, 'detail': error_text, 'error': False}
                    # Sin error visible ni card flip
                    logger.warning(f"[verify][{pin[:8]}...] Timeout sin resultado claro")
                    return {'pin_used': False, 'pin_valid': False, 'detail': 'Timeout sin resultado', 'error': True}

        except Exception as e:
            logger.error(f"[verify][{pin[:8]}...] Error: {e}")
            return {'pin_used': False, 'pin_valid': False, 'detail': str(e), 'error': True}
        finally:
            if context and page:
                await self._return_page(context, page)
            elif page:
